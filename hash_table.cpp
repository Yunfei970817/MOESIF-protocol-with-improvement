...
#include "../protocols/MOSIF_protocol.h"
#include "../protocols/MOESIF_protocol.h"
#include "../protocols/Improved_MOESIF_protocol.h"
#include "settings.h"
#include "sharers.h"
#include "sim.h"
#include "types.h"
...
Hash_entry::Hash_entry (Hash_table *t, paddr_t tag)
{
    ...
    case MESI_PRO:
    	...
    	break;
    case MOSIF_PRO:
    	protocol = new MOSIF_protocol (my_table, this);
    	break;
    case MOESIF_PRO:
        protocol = new MOESIF_protocol (my_table, this);
    	break;
    case Improved_MOESIF_PRO:
        protocol = new Improved_MOESIF_protocol (my_table, this);
    	break;
    default:
...
void Hash_entry::process_request_fetch (Mreq *request)
{
    ...
    protocol->process_fetch_request (request);
}
...
void Hash_table::tick (void)
{
    ...
    if (request)
    {
    	...
    	fprintf(stderr,"*** RECEIVE REQUEST -- ");
...
{
	mreq->src_mid = moduleID;
        mreq->dest_mid = MC_moduleID;
	return this->write_output_port(mreq); // push back to pending
}
/********
 * Debug.
 ********/
